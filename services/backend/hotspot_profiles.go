...
func registerHotspotProfileRoutes(mux *http.ServeMux, admin *administrator, db *sql.DB, worker *workerClient, audit *auditClient) {
	mux.HandleFunc("GET /api/hotspot/profiles", requireSession(admin, func(w http.ResponseWriter, r *http.Request) {
	...
	mux.HandleFunc("POST /api/hotspot/profiles", requireSession(admin, func(w http.ResponseWriter, r *http.Request) {
	...
	mux.HandleFunc("GET /api/hotspot/profiles/{id}", requireSession(admin, func(w http.ResponseWriter, r *http.Request) {
	...
	mux.HandleFunc("PATCH /api/hotspot/profiles/{id}", requireSession(admin, func(w http.ResponseWriter, r *http.Request) {
		id := r.PathValue("id")
		var req hotspotProfileRequest
		if err := json.NewDecoder(r.Body).Decode(&req); err != nil || req.Name == "" {
			http.Error(w, "campo 'name' obrigatorio", http.StatusBadRequest)
	...
	mux.HandleFunc("DELETE /api/hotspot/profiles/{id}", requireSession(admin, func(w http.ResponseWriter, r *http.Request) {
		id := r.PathValue("id")
	...
	mux.HandleFunc("PATCH /api/hotspot/devices/{mac}/profile", requireSession(admin, func(w http.ResponseWriter, r *http.Request) {
