...
func applyLiveBlockForMode(ctx context.Context, db *sql.DB, worker *workerClient, mac, mode string, block bool) {
...
func applyLiveBlockRequest(ctx context.Context, worker *workerClient, iface, mac, mode string, block bool) error {
	if mode == "traffic" {
		ip := ""
		if block {
...
func applyLiveTrafficBlock(ctx context.Context, db *sql.DB, worker *workerClient, mac, ip string, block bool) {
	iface, err := hotspotWifiInterface(ctx, db)
	if err != nil {
...
func applyLiveTrafficBlockRequest(ctx context.Context, worker *workerClient, iface, mac, ip string, block bool) error {
