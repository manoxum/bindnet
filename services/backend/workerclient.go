...
// workerClient fala com o services/worker atraves do socket Unix
...
func newWorkerClient(socketPath string) *workerClient {
	...
	return &workerClient{http: &http.Client{Transport: transport, Timeout: 15 * time.Second}}
}

// call faz uma requisicao para a API interna do worker e decodifica a
// resposta JSON em dest, se dest != nil.
func (c *workerClient) call(ctx context.Context, method, path string, body, dest any) error {
...
func (c *workerClient) callText(ctx context.Context, path string, dest *strings.Builder) error {
